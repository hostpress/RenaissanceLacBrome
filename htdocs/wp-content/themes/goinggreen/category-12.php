...
global $post;
$args = array ('category_name' => 'Conditions du lac','Lake Conditions');
query_posts ($args);
?>
...
?>
  <p><?php printf(__('Statut: %s', 'cater_post'),'<strong>' .get_post_meta($post->ID, 'ecpt_statut', true). '</strong>');?><br />
<?php printf(__('Secchi: %s', 'cater_post'),'<strong>' .get_post_meta($post->ID, 'ecpt_secchi', true). '</strong>');?><br />
<?php printf(__('Transparence: %s', 'cater_post'),'<strong>' .get_post_meta($post->ID, 'ecpt_transparence', true). '</strong>');?><br />
<?php printf(__('Témperature: %s', 'cater_post'),'<strong>' .get_post_meta($post->ID, 'ecpt_temperature', true). '</strong>');?><br />
<?php printf(__('Risque de bloom: %s', 'cater_post'),'<strong>' .get_post_meta($post->ID, 'ecpt_risquedebloom', true). '</strong>');?></p>
